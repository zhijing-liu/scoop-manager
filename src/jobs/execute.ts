...
import type { JobError, JobKind, JobStatus, JobSummary } from './types.js';
import { run, type OutputStream, type RunOptions, type RunResult } from '../services/scoop-runner.js';
...
function createContext(jobId: string): JobContext {
  return {
    jobId,
    log: (text, stream = 'system') => jobManager.log(jobId, stream, text),
    scoop: async (args, options) => {
      return run({
        ...DEFAULT_RUN,
        label: options?.label ?? args.join(' '),
        args,
        jobId,
        serial: options?.serial !== false,
        timeoutMs: options?.timeoutMs,
        onLine: (stream, line) => jobManager.log(jobId, stream, line),
        ...options,
      });
    },
    script: async (body, options) => {
      return run({
        ...DEFAULT_RUN,
